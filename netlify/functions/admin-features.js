...
export async function handler(event) {
  ...
  try {
    ...
    if (!targetOrgId) return { statusCode: 400, headers, body: JSON.stringify({ error: 'orgId required' }) };

    const db = getDb();
    await db`UPDATE organizations SET features = ${JSON.stringify(features)}::jsonb WHERE id = ${targetOrgId}`;
